# -*- coding: utf-8 -*-
"""classification-aerofreno.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1I5niZbNiKjJZ7MoZV4-FLZVN8LYeW0cs
"""
...
file_name = "/content/aerofreno.zip"
...
train_ds = tf.keras.preprocessing.image_dataset_from_directory(
  '/content/aerofreno/train',
  seed=123,
  image_size=(img_height, img_width),
  batch_size=batch_size)

val_ds = tf.keras.preprocessing.image_dataset_from_directory(
  '/content/aerofreno/validation',
  seed=123,
  image_size=(img_height, img_width),
  batch_size=batch_size)
...
predictions = tf.where(predictions < 0.7, 0, 1)
...
for i in range(13):
  ...
  for j in range(len(predictions)):
    ...
    if (p==1) and (t==1):   
      TP=TP+1
    if (p==1) and (t==0):
      missed_cases.append(image_batch[j])
      FP=FP+1
    if (p==0) and (t==0):
      TN=TN+1
    if (p==0) and (t==1):
      considered_cases.append(image_batch[j])
      FN=FN+1
  #next
  if i!=12:
    image_batch, label_batch = tt.next()
...
print('True/Predicted |      n-a      | damaged')
print('-------------------------------')
print('n-a            |      ' + str(TP) + '      |    ' + str(FN))
print('damaged        |        ' + str(FP) + '      |   ' + str(TN))


print('True/Predicted |      n-a      | damaged')
print('-------------------------------')
print('n-a            |      ' + str(100*TP/(TP+FN)) + '      |    ' + str(100*FN/(TP+FN)))
print('damaged        |        ' + str(100*FP/(FP+TN)) + '      |   ' + str(100*TN/(FP+TN)))
...
for i in range(15):
  ax = plt.subplot(4, 4, i + 1)
  plt.imshow(missed_cases[i].astype("uint8"))
  #plt.title(class_names[predictions[i]])
  plt.axis("off")
plt.show()